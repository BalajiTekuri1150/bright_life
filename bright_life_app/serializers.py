...
class ClientUserSerializerWithoutEmail(serializers.ModelSerializer):
  class Meta:
    model = User
    fields = ['id','name','role']
...
class ClientGuardianProfleWithoutMobile(serializers.ModelSerializer):
  user = ClientUserSerializerWithoutEmail(read_only=True)
  class Meta:
    model = Guardian
    fields = ['id','user','organization','profile','source','address','city','state','country','postal_code']
...
class ClientApplicationDetailsSerializer(serializers.ModelSerializer):
  ...
  child_type = ChildTypeSerializer(read_only = True)
  status = ChildStatusSerializer(read_only = True)
  guardian = ClientGuardianProfleWithoutMobile(read_only=True)
  class Meta:
    model = Application
